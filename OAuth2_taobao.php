...
class TAOBAOOAuth {
    ...
    public $host = 'https://eco.taobao.com/router/rest/'; //地址前端

    public static $boundary = ''; //静态 多块的边界
    public static $Referer = 'taobao.com'; //来源
...
function __construct($app_key, $app_secret) {
    ...
    function access_tokenURL() { return 'https://oauth.taobao.com/token'; }
...
function get_authorizeURL($uri, $type, $wicket = NULL, $state = NULL) {
 ...
 switch($wicket){
    case '1':
        $params['view'] = 'popup'; //弹窗小窗口
        break;
    case '2':
        $params['view'] = 'mobile'; //移动终端
        break;
    case '3':
        $params['view'] = 'js'; //微博JS-SDK专用授权页面，弹窗类型，返回结果为JSONP回掉函数
    break;
    default:
        $params['view'] = 'web'; //web浏览器
 }
...
function get_user_id($access, $get_arr){
    $user = Array();
  if(empty($access['taobao_user_id'])){
    $user['id'] = NULL;
  }else{
    $taobao_time = time(); //当前时间
    $user['id'] = $access['taobao_user_id'];
    $user['access_token'] = $access['access_token'];
    $user['expires'] = $taobao_time + $access['expires_in'];
    $user['refresh_token'] = $access['refresh_token'];
    $user['user'] = $access['taobao_user_nick'];
	/**
	[w2_expires_in] => 86400
	[w1_expires_in] => 86400
	[re_expires_in] => 86400
	[r1_expires_in] => 86400
	[r2_expires_in] => 86400
	 [token_type] => Bearer //Access token的类型目前只支持bearer
	 */
  }
    return $user;
} //END get_user_id
